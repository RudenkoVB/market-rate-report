...
Инструменты отчёта — только те, по которым в RMS MOEX заданы ставки риска 1-го уровня (не весь рынок).
История и котировки запрашиваются батчами по списку SECID (см. ISS), без полной постраничной выгрузки рынка.

В блоке «Текущие данные» — marketdata ISS (LAST, MARKETPRICE и др.), батчи с no-cache.
«Итоги торгового дня» — история закрытий по тем же инструментам.
...
GitHub Actions: по умолчанию не качаем длинные названия фьючерсов с www.moex.com (только ISS);
для полных подписей в CI задайте MOEX_FETCH_FUT_TITLES=1.
"""
...
PAGESIZE = 100
# Сколько SECID в одном запросе истории (фильтр securities=) и в батче справочника акций.
HISTORY_BATCH_SECIDS = 55
META_BATCH_SECIDS = 80
EOD_LOOKBACK_DAYS = 45
# Сколько последних торговых дней с успешно собранными ценами закрытия показывать в «Итогах».
EOD_DAYS_TO_SHOW = 5
# Кандидатов дат для «Итогов» (меньше — быстрее прогон).
EOD_CANDIDATE_TRADING_DAYS = 14
# Онлайн-котировки: сколько SECID в одном ISS-запросе marketdata и сколько батчей параллельно.
LIVE_MARKETDATA_CHUNK = 100
...
# Кэш истории в памяти: (рынок, дата, отпечаток универсума SECID).
_HISTORY_MEMORY: Dict[Tuple[str, str, str], Dict[str, Dict[str, Any]]] = {}
...
def prefetch_moex_futures_titles(shortnames: Iterable[str]) -> None:
    ...
    if not need:
        return
    # Сотни HTTP к www.moex.com (не ISS) на холодном CI — десятки минут. По умолчанию пропускаем.
    if os.environ.get("GITHUB_ACTIONS") == "true" or os.environ.get("CI") == "true":
        if os.environ.get("MOEX_FETCH_FUT_TITLES", "").lower() not in ("1", "true", "yes"):
            return

    def fetch_one(s: str) -> Tuple[str, str]:
        try:
            html = fetch_text(moex_contract_page_url(s), timeout=12.0)
...
def fetch_json(url: str, timeout: float = 90.0, retries: int = 3) -> Any:
    ...
    for attempt in range(retries):
        try:
            ...
            if e.code in (429, 502, 503, 504) and attempt < retries - 1:
                time.sleep(min(4.0, 1.4**attempt))
                continue
            raise
...
def fetch_json_live(url: str, timeout: float = 45.0, retries: int = 2) -> Any:
    """
...
def _universe_fingerprint(secids: Set[str]) -> str:
    return hashlib.sha256(",".join(sorted(secids)).encode("utf-8")).hexdigest()[:22]


def _history_cache_path(cfg: MarketConfig, tradedate: str, fp: str) -> Path:
    CACHE_DIR.mkdir(parents=True, exist_ok=True)
    return CACHE_DIR / f"hist_{CACHE_VER}_{cfg.engine}_{cfg.market}_{tradedate}_{fp}.json"


def _filter_history_rec(cfg: MarketConfig, secid: str, rec: Dict[str, Any]) -> bool:
...
def _load_history_one_sec(cfg: MarketConfig, tradedate: str, secid: str) -> Optional[Dict[str, Any]]:
    path = (
        f"/history/engines/{cfg.engine}/markets/{cfg.market}/securities/"
        f"{urllib.parse.quote(secid, safe='')}.json"
    )
    url = f"{ISS}{path}?{urllib.parse.urlencode({'from': tradedate, 'till': tradedate, 'iss.meta': 'off'})}"
    try:
        data = fetch_json(url, timeout=45.0, retries=2)
    except Exception:
        return None
    ...
    for row in rows:
        if row[idx["BOARDID"]] not in cfg.history_boards:
            continue
        if str(row[idx["SECID"]]) != secid:
            continue
        rec = {hcols[i]: row[i] for i in range(len(hcols))}
        if _filter_history_rec(cfg, secid, rec):
            return rec
    return None


def load_history_for_secids(
    cfg: MarketConfig, tradedate: str, secids: Set[str]
) -> Dict[str, Dict[str, Any]]:
    """История только по заданным SECID (батчи securities=), без выгрузки всего рынка."""
    if not secids:
        return {}
    fp = _universe_fingerprint(secids)
    mem_key = (cfg.key, tradedate, fp)
    if mem_key in _HISTORY_MEMORY:
        return _HISTORY_MEMORY[mem_key]
    path = _history_cache_path(cfg, tradedate, fp)
    if path.is_file():
    ...
    sec_list = sorted(secids)
    for i in range(0, len(sec_list), HISTORY_BATCH_SECIDS):
        chunk = sec_list[i : i + HISTORY_BATCH_SECIDS]
        q = urllib.parse.urlencode({"date": tradedate, "iss.meta": "off", "securities": ",".join(chunk)})
        url = f"{ISS}{path_part}?{q}"
        try:
            data = fetch_json(url, timeout=90.0, retries=2)
        except Exception:
            with ThreadPoolExecutor(max_workers=min(16, len(chunk))) as ex:
                futs = {ex.submit(_load_history_one_sec, cfg, tradedate, sid): sid for sid in chunk}
                for fut in as_completed(futs):
                    sid = futs[fut]
                    try:
                        rec = fut.result()
        ...
        for row in rows:
            ...
            if board not in cfg.history_boards:
                continue
            secid = str(row[idx["SECID"]])
            if secid not in secids:
                continue
            rec = {hcols[i]: row[i] for i in range(len(hcols))}
            if not _filter_history_rec(cfg, secid, rec):
                continue
            by_sec[secid] = rec
            got.add(secid)
        missing = set(chunk) - got
        if missing:
            with ThreadPoolExecutor(max_workers=min(16, len(missing))) as ex:
                futs = {ex.submit(_load_history_one_sec, cfg, tradedate, sid): sid for sid in missing}
                for fut in as_completed(futs):
                    sid = futs[fut]
                    try:
                        rec = fut.result()
...
def build_meta_stock_from_limits(stock_lim: Dict[str, float]) -> Dict[str, Any]:
    meta: Dict[str, Any] = {}
    keys = sorted(stock_lim.keys())
    for i in range(0, len(keys), META_BATCH_SECIDS):
        chunk = keys[i : i + META_BATCH_SECIDS]
        q = urllib.parse.urlencode(
            {
                "iss.meta": "off",
                "securities": ",".join(chunk),
                "securities.columns": "SECID,SHORTNAME,SECNAME,BOARDID",
            }
        )
        try:
            data = fetch_json(f"{ISS}/engines/stock/markets/shares/securities.json?{q}", timeout=60.0, retries=2)
        except Exception:
            continue
        _, rows = iss_table_rows(data, "securities")
        scols = data.get("securities", {}).get("columns") or []
        ...
        si = {c: i for i, c in enumerate(scols)}
        for row in rows:
            if row[si["BOARDID"]] != "TQBR":
                continue
            sid = str(row[si["SECID"]])
            if sid not in stock_lim:
                continue
            meta[sid] = {
                "SHORTNAME": row[si["SHORTNAME"]],
                "SECNAME": row[si["SECNAME"]] if "SECNAME" in si else "",
...
def build_meta_currency_from_limits(cur_lim: Dict[str, float]) -> Dict[str, Any]:
    meta: Dict[str, Any] = {}
    for asset in cur_lim.keys():
        a = str(asset).strip().upper()
        if len(a) < 3:
            continue
        sid = f"{a}RUB_TOM"
        meta[sid] = {"SHORTNAME": sid, "SECNAME": ""}
    return meta


def _fut_asset_allowed(ac: Any, fut_lim: Dict[str, float]) -> bool:
    if ac is None:
        return False
    s = str(ac).strip()
    for k in fut_lim.keys():
        if str(k).strip().upper() == s.upper():
            return True
    return False


def build_meta_futures_from_limits(fut_lim: Dict[str, float]) -> Dict[str, Any]:
    meta: Dict[str, Any] = {}
    if not fut_lim:
        return meta
    start = 0
    while True:
        q = urllib.parse.urlencode(
            {
                "iss.meta": "off",
                "start": str(start),
                "securities.columns": "SECID,SHORTNAME,SECNAME,BOARDID,ASSETCODE",
            }
        )
        data = fetch_json(f"{ISS}/engines/futures/markets/forts/securities.json?{q}", timeout=60.0, retries=2)
        _, rows = iss_table_rows(data, "securities")
        if not rows:
            break
        scols = data.get("securities", {}).get("columns") or []
        ...
        for row in rows:
            ...
            sn = str(row[si["SHORTNAME"]] or "")
            if not _fut_asset_allowed(ac, fut_lim):
                continue
            if not is_futures_contract(sn, ac):
                continue
            sid = str(row[si["SECID"]])
            meta[sid] = {
                "SHORTNAME": sn,
...
def live_meta_for_report(
    cfg: MarketConfig,
    stock_lim: Dict[str, float],
    cur_lim: Dict[str, float],
    fut_lim: Dict[str, float],
) -> Dict[str, Any]:
    """Справочник инструментов только из RMS (ставки риска 1 ур.)."""
    if cfg.key == "stock":
        return build_meta_stock_from_limits(stock_lim)
    if cfg.key == "currency":
        return build_meta_currency_from_limits(cur_lim)
    return build_meta_futures_from_limits(fut_lim)
...
def live_market_block(
    ...
    """
    if meta is None:
        meta = live_meta_for_report(cfg, stock_lim, cur_lim, fut_lim)
...
def build_table_rows(
    ...
    h0 = load_history_for_secids(cfg, d0, universe_secids)
    if h1 is None:
        h1 = load_history_for_secids(cfg, d1, universe_secids)
    if h2 is None:
        h2 = load_history_for_secids(cfg, d2, universe_secids)
...
def _eod_dates_for_preload(cfg: MarketConfig, eod_candidates: List[str]) -> Set[str]:
    """Торговые даты для EOD-блока (кандидаты с валидным тройным днём)."""
    out: Set[str] = set()
...
def _build_market_report_slice(
    ...
    meta = live_meta_for_report(cfg, stock_lim, cur_lim, fut_lim)
    universe = set(meta.keys())
    d0, d1, d2 = get_three_trading_days(cfg, today)
    ref1, ref2 = live_comparison_close_dates(cfg, today)
    dates_needed: Set[str] = {d0, d1, d2, ref1, ref2} | _eod_dates_for_preload(cfg, eod_candidates)
    for td in sorted(dates_needed):
        load_history_for_secids(cfg, td, universe)
    h1 = load_history_for_secids(cfg, ref1, universe)
    h2 = load_history_for_secids(cfg, ref2, universe)
    md_map, meta = live_market_block(cfg, stock_lim, cur_lim, fut_lim, meta=meta)
    current_entry = {
        "title": cfg.title,
        "basis": (
