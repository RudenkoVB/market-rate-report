...
def collect_eod_report_dates(stock_cfg: MarketConfig, today: date, max_sessions: int) -> List[str]:
    ...
    out: List[str] = []
    seen: set = set()
    for delta in range(EOD_DAYS_BACK + 60):
...
def generate_report() -> Path:
    ...
    eod_report_dates = collect_eod_report_dates(stock_cfg, today, EOD_DAYS_BACK + 1)
...
<body>
  ...
  let DATA = ensureDataShape(parseEmbeddedPayload());
  ...
  function safeFilename(s) {
    return String(s).replace(/[^a-zA-Z0-9._А-Яа-яЁё-]+/g, '_').slice(0, 80);
  }
  ...
  if (canPoll) setTimeout(pullLatestData, 1500);
