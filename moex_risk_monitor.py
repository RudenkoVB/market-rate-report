...
Инструменты отчёта — ликвидные инструменты рынков TQBR / CETS TOM / FORTS (как в прежней версии отчёта).
История закрытий — постраничная выгрузка ISS по рынку на дату (кэш в moex_cache/).

В блоке «Текущие данные» — marketdata ISS: лучшие цены стакана BID/OFFER (середина спреда), иначе LAST и др.; запросы без кэша на клиенте.
...
# Кэш истории в памяти: (ключ рынка, дата, метка кэша — полный срез рынка full).
...
def load_history_cached(cfg: MarketConfig, tradedate: str) -> Dict[str, Dict[str, Any]]:
    """История закрытий по рынку на дату: постраничный ISS history (как в прежней версии отчёта)."""
    fp = "full"
    ...
    start = 0
    while True:
        q = urllib.parse.urlencode({"date": tradedate, "iss.meta": "off", "start": str(start)})
        url = f"{ISS}{path_part}?{q}"
        data = fetch_json(url, timeout=90.0, retries=3)
...
def live_securities_meta(cfg: MarketConfig) -> Dict[str, Any]:
    """Справочник инструментов для «Текущих данных» (securities, постранично по рынку)."""
    if cfg.key == "currency":
    ...
    q = urllib.parse.urlencode(
        {
            ...
            "securities.columns": "SECID,BOARDID",
            "marketdata.columns": (
                "SECID,BOARDID,BID,OFFER,LAST,MARKETPRICE,LCURRENTPRICE,WAPRICE,"
...
def live_market_block(cfg: MarketConfig) -> Tuple[Dict[str, Dict[str, Any]], Dict[str, Any]]:
    ...
    Цены: лучшие BID/OFFER (стакан), затем LAST и др. Батчи securities= через fetch_json_live.
    """
...
def current_price_live(md: Dict[str, Any]) -> Optional[float]:
    """Текущая рыночная цена: середина спреда стакана (BID/OFFER), иначе последняя сделка и оценки ISS."""
    bid = md.get("BID")
    offer = md.get("OFFER")
    try:
        fb = float(bid) if bid is not None else None
    except (TypeError, ValueError):
        fb = None
    try:
        fo = float(offer) if offer is not None else None
    except (TypeError, ValueError):
        fo = None
    if fb is not None and fo is not None and fb > 0 and fo > 0:
        return (fb + fo) / 2.0
...
def _build_market_report_slice(
    ...
    current_entry = {
        ...
        "basis": (
            ...
            f"Текущая цена — по стакану ISS (BID/OFFER), при отсутствии — последняя сделка и др.; снимок на момент запроса."
        ),
